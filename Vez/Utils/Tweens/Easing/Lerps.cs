...
namespace Vez.Utils.Tweens.Easing
{
    /// <summary>
    ...
    public static class Lerps
	{
		...
		public static float LerpDamp(float deltaTime, float source, float target, float smoothing)
		{
			return MathHelper.Lerp(source, target, 1 - Mathf.Pow(smoothing, deltaTime));
		}
		...
		public static float FastSpring(float deltaTime, float currentValue, float targetValue, ref float velocity, float dampingRatio,
		                               float angularFrequency)
		{
			velocity += -2.0f * deltaTime * dampingRatio * angularFrequency * velocity +
			            deltaTime * angularFrequency * angularFrequency * (targetValue - currentValue);
		...
		/// <param name="angularFrequency">An angular frequency of 2pi (radians per second) means the oscillation completes one
		/// full period over one second, i.e. 1Hz. should be less than 35 or so to remain stable</param>
		public static float StableSpring(float deltaTime, float currentValue, float targetValue, ref float velocity, float dampingRatio,
		                                 float angularFrequency)
		{
			var f = 1f + 2f * deltaTime * dampingRatio * angularFrequency;
			var oo = angularFrequency * angularFrequency;
			var hoo = deltaTime * oo;
			var hhoo = deltaTime * hoo;
			var detInv = 1.0f / (f + hhoo);
			var detX = f * currentValue + deltaTime * velocity + hhoo * targetValue;
		...
		/// <param name="angularFrequency">An angular frequency of 2pi (radians per second) means the oscillation completes one
		/// full period over one second, i.e. 1Hz. should be less than 35 or so to remain stable</param>
		public static Vector2 FastSpring(float deltaTime, Vector2 currentValue, Vector2 targetValue, ref Vector2 velocity,
		...
		/// <param name="angularFrequency">An angular frequency of 2pi (radians per second) means the oscillation completes one
		/// full period over one second, i.e. 1Hz. should be less than 35 or so to remain stable</param>
		public static Vector2 StableSpring(float deltaTime, Vector2 currentValue, Vector2 targetValue, ref Vector2 velocity,
		                                   float dampingRatio, float angularFrequency)
		{
			var f = 1f + 2f * deltaTime * dampingRatio * angularFrequency;
			var oo = angularFrequency * angularFrequency;
