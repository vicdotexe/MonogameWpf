...
namespace Vez.Utils.Extensions
{
    public static class SpriteBatchExt
    {
        ...
        private static Sprite _pixelTexture(SpriteBatch spriteBatch)
        {
            if (!_pixelTextures.TryGetValue(spriteBatch.GraphicsDevice, out var sprite))
            {
                var texture = new Texture2D(spriteBatch.GraphicsDevice, 1, 1);
                texture.SetData(new[] { Color.White });
                sprite = new Sprite(texture);
                _pixelTextures.Add(spriteBatch.GraphicsDevice, sprite);
            }
        ...
        public static void DrawLineAngle(this SpriteBatch spriteBatch, Vector2 start, float angle, float length,
                                         Color color)
        {
            spriteBatch.Draw(_pixelTexture(spriteBatch), start, _pixelTexture(spriteBatch).SourceRect, color,
                                    ...
                                    Color color)
        {
            ...
            spriteBatch.Draw(_pixelTexture(spriteBatch), _tempRect, _pixelTexture(spriteBatch).SourceRect,
                color);
        }
        ...
        {
            _tempRect = rect;
            spriteBatch.Draw(_pixelTexture(spriteBatch), rect, _pixelTexture(spriteBatch).SourceRect, color);
        }
        ...
        {
            var sourceRect = _pixelTexture(spriteBatch).SourceRect;
            if (size != 1)
            {
            ...
            spriteBatch.Draw(_pixelTexture(spriteBatch), position, sourceRect, color);
        }
