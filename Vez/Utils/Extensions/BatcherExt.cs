...
namespace Vez.Utils.Extensions
{
    ...
    public static class BatcherDrawingExt
    {
        ...
        private static Sprite _pixelTexture(Batcher batcher)
        {
            if (!_pixelTextures.TryGetValue(batcher.GraphicsDevice, out var sprite))
            {
                var texture = new Texture2D(batcher.GraphicsDevice, 1, 1);
                texture.SetData(new[] { Color.White });
                sprite = new Sprite(texture);
                _pixelTextures.Add(batcher.GraphicsDevice, sprite);
            }
        ...
        {
            batcher.Draw(_pixelTexture(batcher), start, _pixelTexture(batcher).SourceRect, color,
        ...
        public static void DrawRect(this Batcher batcher, float x, float y, float width, float height, Color color)
        {
            ...
            batcher.Draw(_pixelTexture(batcher), _tempRect, _pixelTexture(batcher).SourceRect, color);
        }
        ...
        {
            batcher.Draw(_pixelTexture(batcher), rect, _pixelTexture(batcher).SourceRect, color);
        }
        ...
        public static void DrawPixel(this Batcher batcher, Vector2 position, Color color, int size = 1)
        {
            ...
            batcher.Draw(_pixelTexture(batcher), destRect, _pixelTexture(batcher).SourceRect, color);
        }
