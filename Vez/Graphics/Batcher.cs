...
namespace Vez.Graphics
{
    public class Batcher : GraphicsResource
    {
        ...
        Vez.Graphics.SpriteEffect _spriteEffect;
        ...
        public Batcher(GraphicsDevice graphicsDevice)
        {
            ...
            using (var stream = assembly.GetManifestResourceStream(resources.Last()))
            {
                using (var ms = new MemoryStream())
                {
                    stream.CopyTo(ms);
                    _spriteEffect = new Vez.Graphics.SpriteEffect(GraphicsDevice, ms.ToArray());
                }
            }
        ...
        public void Begin()
        {
            Begin(BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.None,
